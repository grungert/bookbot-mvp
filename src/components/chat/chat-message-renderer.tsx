      ...
      >
        ...
        {parsed.ui && (
          <div className={cn(!isUser && "px-2 pb-2")}>
            ...
            {/* Fallback: unknown component - don't render UI, just log for debugging */}
            {!["service-selector", "date-picker", "time-slots", "booking-card"].includes(
              parsed.ui.component
            ) &&
              console.warn(
                `ChatMessageRenderer: Unknown UI component "${parsed.ui.component}" - not rendering`
              )}
          </div>
        )}
      </div>

...
import { viberSceneTiming } from "../viberConstants";

// Viber purple-themed settings
const SETTINGS = {
  ...
  // Viber purple gradient (center/base - start)
  colorA: "#7360F2",        // Viber purple
  colorB: "#9F7AEA",        // Lighter purple
  leaderColor: "#7360F2",
  leaderLightColor: "#5A4BC0",

  // colors (end - at full progress)
  colorAEnd: "#6B5CE7",
  colorBEnd: "#805AD5",     // Medium purple
  leaderColorEnd: "#6B5CE7",
  leaderLightColorEnd: "#4A3D9E",

  // colors (left - light purple dominant)
  colorALeft: "#A78BFA",    // Light violet
  colorBLeft: "#C4B5FD",    // Lavender
  leaderColorLeft: "#8B5CF6",
  leaderLightColorLeft: "#6D28D9",

  // colors (right - deep purple dominant)
  colorARight: "#5B21B6",   // Deep purple
  colorBRight: "#4C1D95",   // Very deep purple
  leaderColorRight: "#6D28D9",
...
// Derive scene timing
const SCENES = {
  hook: { start: viberSceneTiming.hook.start, end: viberSceneTiming.hook.start + viberSceneTiming.hook.duration },
  hero: { start: viberSceneTiming.hero.start, end: viberSceneTiming.hero.start + viberSceneTiming.hero.duration },
  demo: { start: viberSceneTiming.demo.start, end: viberSceneTiming.demo.start + viberSceneTiming.demo.duration },
  features: { start: viberSceneTiming.features.start, end: viberSceneTiming.features.start + viberSceneTiming.features.duration },
  useCases: { start: viberSceneTiming.useCases.start, end: viberSceneTiming.useCases.start + viberSceneTiming.useCases.duration },
  cta: { start: viberSceneTiming.cta.start, end: viberSceneTiming.cta.start + viberSceneTiming.cta.duration },
};
...
function computeLeaderPath(totalFrames: number, fps: number): THREE.Vector3[] {
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    if (frame < SCENES.hook.end) {
      ...
      // Demo: Stay center-left (purple visible through glass panel)
      ...
      // Features: Move from center to right (deeper purple becomes visible)
      ...
      // UseCases: Stay on right side (deep purple dominant)
  ...
  <>
    ...
    <div
      style={{
        ...
        inset: 0,
        backgroundImage: `
          linear-gradient(to right, rgba(115, 96, 242, 0.1) 1px, transparent 1px),
          linear-gradient(to bottom, rgba(115, 96, 242, 0.1) 1px, transparent 1px)
        `,
        backgroundSize: "60px 60px",
        filter: "drop-shadow(0 0 3px rgba(115, 96, 242, 0.4))",
    ...
    <div
      style={{
        ...
        inset: 0,
        background: `
          radial-gradient(ellipse 80% 60% at 0% 0%, rgba(115, 96, 242, 0.12) 0%, transparent 50%),
          radial-gradient(ellipse 50% 40% at 100% 100%, rgba(90, 75, 192, 0.08) 0%, transparent 50%)
        `,
      }}
    />
  </>
...
export const ViberBackground: React.FC = () => {

...
import { mobileSceneTiming } from "../mobileConstants";

// Mobile blue/purple themed settings
const SETTINGS = {
  ...
  // Mobile blue/purple gradient (center/base - start)
  colorA: "#3b82f6",        // Blue-500
  colorB: "#a855f7",        // Purple-500
  leaderColor: "#3b82f6",
  leaderLightColor: "#2563eb",

  // colors (end - at full progress)
  colorAEnd: "#2563eb",     // Blue-600
  colorBEnd: "#9333ea",     // Purple-600
  leaderColorEnd: "#2563eb",
  leaderLightColorEnd: "#1d4ed8",

  // colors (left - light blue dominant)
  colorALeft: "#60a5fa",    // Blue-400
  colorBLeft: "#c4b5fd",    // Purple-200
  leaderColorLeft: "#3b82f6",
  leaderLightColorLeft: "#2563eb",

  // colors (right - purple dominant)
  colorARight: "#7c3aed",   // Violet-600
  colorBRight: "#6d28d9",   // Violet-700
  leaderColorRight: "#8b5cf6",
  leaderLightColorRight: "#7c3aed",
...
// Derive scene timing
const SCENES = {
  hook: { start: mobileSceneTiming.hook.start, end: mobileSceneTiming.hook.start + mobileSceneTiming.hook.duration },
  hero: { start: mobileSceneTiming.hero.start, end: mobileSceneTiming.hero.start + mobileSceneTiming.hero.duration },
  demo: { start: mobileSceneTiming.demo.start, end: mobileSceneTiming.demo.start + mobileSceneTiming.demo.duration },
  features: { start: mobileSceneTiming.features.start, end: mobileSceneTiming.features.start + mobileSceneTiming.features.duration },
  useCases: { start: mobileSceneTiming.useCases.start, end: mobileSceneTiming.useCases.start + mobileSceneTiming.useCases.duration },
  cta: { start: mobileSceneTiming.cta.start, end: mobileSceneTiming.cta.start + mobileSceneTiming.cta.duration },
};
...
function computeLeaderPath(totalFrames: number, fps: number): THREE.Vector3[] {
  ...
  for (let frame = 0; frame < totalFrames; frame++) {
    ...
    if (frame < SCENES.hook.end) {
      ...
      // Demo: Stay center-left (blue visible through glass panel)
  ...
  <>
    ...
    <div
      style={{
        ...
        inset: 0,
        background: `
          radial-gradient(ellipse 80% 60% at 0% 0%, rgba(59, 130, 246, 0.12) 0%, transparent 50%),
          radial-gradient(ellipse 50% 40% at 100% 100%, rgba(168, 85, 247, 0.08) 0%, transparent 50%)
        `,
      }}
    />
  </>

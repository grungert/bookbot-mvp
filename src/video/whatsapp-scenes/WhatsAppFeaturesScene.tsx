...
// Get gradient color based on grid position (WhatsApp green gradient)
function getWhatsAppGradientColor(index: number): { bg: string; icon: string } {
  const factor = index / 3;
  // Interpolate from primary green to secondary teal
  const r = Math.round(37 + (18 - 37) * factor);
  const g = Math.round(211 + (140 - 211) * factor);
  const b = Math.round(102 + (126 - 102) * factor);
...
const FeatureCard: React.FC<{
  ...
  const gradientColors = getWhatsAppGradientColor(index);
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 24,
        padding: 32,
        boxShadow: `0 15px 35px ${whatsappColors.primary}15`,
...
export const WhatsAppFeaturesScene: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  ...
  const features = [
    ...
    {
      title: t.features.items.reach.title,
      description: t.features.items.reach.description,
      icon: <GlobeIcon />,
      delay: 75,
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t.features.title} <WhatsAppGradientText>{t.features.titleHighlight}</WhatsAppGradientText>
        </h2>
      </div>
